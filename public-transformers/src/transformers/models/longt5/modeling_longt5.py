...
class LongT5LocalAttention(nn.Module):
    ...
    def forward(
        ...
        # Compute scores
        # Einsum meaning: For each head, compute Q (qxh) * K^T (hxk).
        scores = torch.einsum(
            "...qhd,...khd->...hqk", query_states, key_states
        )  # (batch_size, num_block, n_heads, block_len, 3 * block_len)
        # The einsum can be rewritten using matmul
        # query_states = torch.permute([0...n-3, n-1,n-2,n]) -> ...hqd
        # key_states = torch.permute([0...n-3, n-1,n,n-2]) -> ...hdk
        # scores = torch.matmul(query_states, key_states) -> ...hqk
        ...
        # Einsum meaning: multiply by value matrix
...
class LongT5MemoryAttention(nn.Module):
    def __init__(self, config: LongT5Config, has_relative_attention_bias=False):
        super().__init__()
        self.is_decoder = config.is_decoder
        self.has_relative_attention_bias = has_relative_attention_bias
    ...
    def forward(
        ...
        mem_kv, mem_mask, retrieved_tokens = knn_memories.search(no_batch_query_states, self.memory_topk) # (1, batch_size * n_heads, seq_len, topk, 2, dim_head)
        mem_kv = rearrange(
        ...
        logger.info('token decode: {}'.format(tokenizer.decode(top_token)[:display_decode]))
    ...
    def forward(
        ...
        for i, (layer_module, past_key_value) in enumerate(zip(self.block, past_key_values)):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                # if i in knn_memory_layer and knn_memories is not None:
...
class LongT5ForConditionalGeneration(LongT5PreTrainedModel):
    ...
    def __init__(self, config: LongT5Config):
        ...
        self.memory_layers = knn_memory_layer
    ...
    def forward(
        ...
        with self.knn_memories_context(batch_size = 1) as knn_memories:
            logger.info('true seq: {}'.format(tokenizer.decode(decoder_input_ids[0][:display_decode])))
            decoder_outputs = self.decoder(
                input_ids=decoder_input_ids,
                attention_mask=decoder_attention_mask,
                inputs_embeds=decoder_inputs_embeds,
    ...
    def prepare_inputs_for_generation(
        ...
        past=None,
        attention_mask=None, # same as input_ids
        head_mask=None,
